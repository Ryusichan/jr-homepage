...
const projects: ProjectData[] = [
  ...
  },
  {
    ...
    client: "고용보조금 지원",
    description:
      "복잡하고 번거로웠던 고용보조금 신청 과정을 디지털로 전환하여 기업과 고용주가 손쉽게 지원금을 신청하고 관리할 수 있도록 돕는 플랫폼입니다. 기존에는 여러 기관을 방문하고 수많은 서류를 준비해야 했던 절차를 온라인에서 원스톱으로 처리할 수 있도록 구현하여 신청자의 시간과 비용 부담을 크게 줄였습니다. 기업 규모, 업종, 고용 형태 등 기본 정보를 입력하면 해당 기업이 받을 수 있는 다양한 고용보조금을 자동으로 안내하며, 각 지원 사업별 자격 요건과 지원 금액을 명확하게 비교할 수 있어 합리적인 선택이 가능합니다. 신청서 작성 과정에서는 단계별 가이드와 실시간 유효성 검증을 통해 오류를 최소화하고, 필요한 서류를 자동으로 체크리스트로 제공하여 누락 없이 신청을 완료할 수 있도록 지원합니다. 관리자 대시보드에서는 신청 현황, 심사 진행 상태, 승인/반려 내역을 실시간으로 확인할 수 있으며, 자동 알림 시스템을 통해 중요한 상태 변경을 즉시 전달받을 수 있습니다. 상담 예약과 전문가 문의 기능을 통해 신청 과정에서 궁금한 점을 빠르게 해결할 수 있으며, 반응형 디자인을 적용하여 데스크톱과 모바일 모든 환경에서 최적화된 사용 경험을 제공합니다. 안정적인 클라우드 인프라와 보안 프로토콜을 적용하여 민감한 기업 정보를 안전하게 보호하며, 향후 신규 지원 사업 추가에도 유연하게 대응할 수 있는 확장성 있는 구조로 개발되었습니다.",
    techStack: ["Next.js", "TypeScript", "Tailwind CSS", "AWS"],
  ...
  },
  {
    ...
    client: "업무 관리",
    description:
      "기업의 복잡한 업무 프로세스를 디지털로 체계화하여 조직의 운영 효율을 극대화하는 업무 관리 플랫폼입니다. 업무 등록, 진행 관리, 담당자 배정, 마감 일정 추적, 협업 소통까지 조직 운영에 필요한 핵심 기능을 하나의 플랫폼에서 통합 제공하여 여러 도구를 오가는 번거로움을 제거했습니다. 실시간 대시보드를 통해 부서별·프로젝트별 진행 현황을 한눈에 파악할 수 있으며, 병목 지점과 지연 업무를 즉시 식별하여 의사결정 속도를 높입니다. 직관적인 UI와 드래그 앤 드롭 기반의 인터랙션으로 업무 상태 변경, 우선순위 조정, 담당자 재배정 등을 손쉽게 처리할 수 있으며, 자동 알림 시스템으로 중요한 업무 변경 사항을 관련 구성원에게 즉시 전달합니다. 권한 기반 접근 제어를 통해 조직 구조와 역할에 맞춘 맞춤형 운영 환경을 구성할 수 있으며, 상세한 감사 로그로 모든 업무 활동을 추적하여 투명성과 책임성을 확보했습니다. 업무 데이터 분석 리포트 기능을 통해 구성원별 생산성, 프로젝트 완료율, 평균 처리 시간 등 핵심 지표를 시각화하여 데이터 기반의 운영 개선이 가능합니다. 반응형 디자인을 적용하여 데스크톱, 태블릿, 모바일 모든 환경에서 끊김 없는 업무 처리가 가능하며, 안정적인 클라우드 인프라 위에서 운영되어 대규모 조직에서도 원활한 성능을 보장합니다.",
    techStack: ["Next.js", "TypeScript", "Tailwind CSS", "AWS"],
